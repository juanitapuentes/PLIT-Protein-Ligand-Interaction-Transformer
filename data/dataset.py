...
def transform_molecule_pg(
    ...
    if is_prot:
        ...
        if args.mode == "demo":
            return Data(edge_attr=edge_attr, edge_index=edge_index, x=x, y=y, ptr= [1,2], mol=mol, smiles=smiles)
...
def demo_molecule(raw_smiles, args):
    path = "data/datasets/DUDE/"
    data_test = pd.read_csv(
        path + "Smiles_Test.csv", names=["Smiles", "Target", "Label"]
    )
    df_row = data_test.loc[data_test['Smiles'] == raw_smiles]
    for mol, label in zip(df_row["Smiles"], df_row["Label"]):
        Data = transform_molecule_pg(mol, label, args, saliency=args.saliency)
